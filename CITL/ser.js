...
h: app.use(bodyParser.urlencoded({ extended: true }));
...
app.post("/registration", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Registration successful", user: response });
  } catch (error) {
  ...
  try {
    ...
    newLocation
      ...
      .catch((error) => {
        ...
        res
          .status(500)
          .json({ error: "Error saving location: " + error.message });
      });
  } catch (error) {
...
app.post("/add_to_cart", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Item added to cart successfully", cartItem: cartItem });
  } catch (error) {
